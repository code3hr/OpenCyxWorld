...
1. Run `pnpm dev` and try generating a scenario such as “Show me a product launch walkthrough” or “Guide the first-week onboarding lab.”  
2. Look at the generated JSON to ensure there’s no “classroom” language—just the experience you wanted.  
3. If you create new prompt directories, plug them in via `PROMPT_IDS` or a config flag without changing the rendering layer.
...
- Use the demo script (`scripts/demo-open-world.ts`) to log the prompts and outlines for each scenario; replace the stubbed AI call with a provider once you add API keys.
